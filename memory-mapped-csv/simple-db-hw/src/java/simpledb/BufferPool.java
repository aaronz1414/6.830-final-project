...
public class BufferPool {
    ...
    private Queue<PageId> pageOrder;
    ...
    public Page getPage(TransactionId tid, PageId pid, Permissions perm)
    	...
    	if (Permissions.READ_ONLY.equals(perm)) {
    		success = lockManager.acquireLock(pid, tid, false);
    	} else if (Permissions.READ_WRITE.equals(perm)) {
    		success = lockManager.acquireLock(pid, tid, true);
    	}
    ...
    public void transactionComplete(TransactionId tid, boolean commit)
    	...
    	List<PageId> readPids = lockManager.getPageIds(tid, false);
    	List<PageId> writePids = lockManager.getPageIds(tid, true);
    	
    	if (commit) {
    		for (PageId pid : writePids) {
    			if (pages.containsKey(pid)) {
        			flushPage(pid);
        			pages.get(pid).markDirty(false, tid);
        		}
    		}
    	} else {
    		for (PageId pid : writePids) {
    	...
    	lockManager.release(readPids, tid);
    	lockManager.release(writePids, tid);
    }
    ...
    */
    ...
    public synchronized  void flushPages(TransactionId tid) throws IOException {
    	...
    	List<PageId> pids = lockManager.getPageIds(tid, true);
    ...
    private synchronized  void evictPage() throws DbException {
        ...
        try {
        	ArrayList<PageId> removed = new ArrayList<>();
        	PageId pageId = pageOrder.remove();
        	while (pages.get(pageId).isDirty() != null) {
        		removed.add(pageId);
        		pageId = pageOrder.poll();
        		if (pageId == null) throw new DbException("All pages are dirty");
        	}
        	
        	List<PageId> remaining = new ArrayList<>(pageOrder);
        	pageOrder = new LinkedList<>();
        	pageOrder.addAll(removed);
        	pageOrder.addAll(remaining);
