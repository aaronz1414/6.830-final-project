...
public class JoinOptimizer {
    ...
    public Vector<LogicalJoinNode> orderJoins(
    	...
    	for (int i = 1; i <= joins.size(); i++) {
    		for (Set<LogicalJoinNode> subjoin : enumerateSubsets(joins, i)) {
    			
    			Vector<LogicalJoinNode> bestPlan = new Vector<>();
    			int cardOfBestPlan = 0;
    			double bestCostSoFar = Double.MAX_VALUE;
    			
    			for (LogicalJoinNode joinToRemove : subjoin) {
    				...
    				if (costCard != null) {
    					...
    					cardOfBestPlan = costCard.card;
    					bestCostSoFar = costCard.cost;
    				}
    			}
    			
    			planCache.addPlan(subjoin, bestCostSoFar, cardOfBestPlan, bestPlan);
    		}
    	}
    	
    	if (explain) {
    		printJoins(planCache.getOrder(new HashSet<>(joins)), 
                    planCache,
                    stats,
                    filterSelectivities);
    	}
